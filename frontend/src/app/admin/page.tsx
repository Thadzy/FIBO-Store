...
export default function AdminDashboard() {
  ...
  const handleDeleteItem = async (id: number) => {
    ...
    try {
        ...
        fetchData();
    } catch (error) { alert("ลบไม่สำเร็จ: อุปกรณ์อาจถูกจองอยู่"); }
  };
  ...
  if (status === "loading" || session?.user?.role !== "admin") return <div>Loading...</div>;
    ...
    <div className="min-h-screen bg-slate-100 text-blue-900">
      ...
      <nav className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center shadow-sm sticky top-0 z-40">
        ...
        <div className="flex gap-3">
            {activeTab === "inventory" && (
                 <button onClick={() => setIsAddModalOpen(true)} className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold px-4 py-2 rounded-lg shadow-md transition-all active:scale-95 flex items-center gap-2">
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4"/></svg>
                    Add Item
                </button>
            )}
            <button onClick={fetchData} className="text-sm text-slate-500 hover:text-blue-600 font-bold border border-slate-200 px-3 py-2 rounded-lg hover:bg-slate-50 flex items-center gap-2">
      ...
      <main className="max-w-7xl mx-auto p-6">
        ...
        <div className="flex space-x-1 bg-white p-1 rounded-xl shadow-sm border border-slate-200 mb-6 w-fit">
            <button 
                ...
                className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all ${activeTab === "bookings" ? "bg-slate-800 text-white shadow" : "text-slate-500 hover:bg-slate-50"}`}
            >
                📋 Booking Requests
            </button>
            <button 
                onClick={() => setActiveTab("inventory")}
                className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-all ${activeTab === "inventory" ? "bg-slate-800 text-white shadow" : "text-slate-500 hover:bg-slate-50"}`}
            >
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {bookings.map((b) => (
                                <tr key={b.booking_id} className="hover:bg-slate-50">
                                    ...
                                    <td className="px-6 py-4 font-bold text-slate-700">{b.user_name}</td>
                                    <td className="px-6 py-4">
                                        {b.items.map((i,idx) => <div key={idx}>• {i.name} x{i.quantity}</div>)}
                                    </td>
                                    <td className="px-6 py-4">{getStatusBadge(b.status)}</td>
                                    <td className="px-6 py-4">
                                        <div className="flex justify-center items-center gap-2">
                                            {/* ✅ ปุ่ม Approve/Reject แบบใหม่ */}
                                            {b.status === "Pending" && (
                                                <>
                                                    <button 
                            ...
                            {items.map((item) => (
                                <tr key={item.item_id} className="hover:bg-slate-50 group transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        ...
                                        <span className="text-xs text-slate-400 ml-1">{item.specifications['unit']}</span>
                                    </td>
      ...
      <AddItemModal 
        ...
        onSuccess={() => { fetchData(); alert("เพิ่มสำเร็จ!"); }} 
      />
      ...
      <EditItemModal
        ...
        item={editingItem}
        onSuccess={() => { fetchData(); alert("แก้ไขสำเร็จ!"); }}
      />
    </div>
  );
}