    ...
    <>
      ...
      <div className={`fixed inset-y-0 right-0 z-50 w-full max-w-md bg-white shadow-2xl transform transition-transform duration-300 ease-out flex flex-col ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start">
                        ...
                        <button onClick={() => onRemoveItem(item.item_id)} className="text-slate-400 hover:text-red-500 transition-colors">
                  ...
                  <div className="flex items-center justify-between mt-2">
                    <div className="flex items-center border border-slate-200 rounded-lg bg-slate-50">
                        <button onClick={() => onDecreaseItem(item.item_id)} disabled={item.borrow_qty <= 1} className="px-3 py-1 text-slate-600 hover:bg-slate-200 rounded-l-lg disabled:opacity-30 disabled:hover:bg-transparent">-</button>
                        <span className="px-2 text-sm font-bold text-slate-800 min-w-[30px] text-center">{item.borrow_qty}</span>
                        <button onClick={() => onIncreaseItem(item.item_id)} disabled={item.borrow_qty >= item.available_quantity} className="px-3 py-1 text-slate-600 hover:bg-slate-200 rounded-r-lg disabled:opacity-30 disabled:hover:bg-transparent">+</button>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="p-6 border-t border-slate-100 bg-slate-50">
          <button onClick={onCheckout} disabled={cartItems.length === 0} className="w-full bg-orange-500 hover:bg-orange-600 disabled:bg-slate-300 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-orange-200 disabled:shadow-none transition-all active:scale-95 flex items-center justify-center gap-2">
            <span>Confirm Requisition</span>
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
          </button>
          <p className="text-center text-[10px] text-slate-400 mt-3">By confirming, you agree to return items within 7 days.</p>
        </div>
      </div>
    </>
  );
